...
function BookAppointment({ room }: { room: any }) {
  ...
  async function getEvents() {
    const apiUrl = 'http://10.10.12.95:3001/api/admin/gettimeroom';
    try {
  ...
  async function AddTimeAppointment(
    ...
    const apiUrl = "http://10.10.12.95:3001/api/appointment/addtimeappointment";
    try {
  ...
  const appointment = async (studentid: string) => {
    ...
    try {
      const response = await axios.put(
        "http://10.10.12.95:3001/api/appointment/checkappointment",
        { studentid }
      );
